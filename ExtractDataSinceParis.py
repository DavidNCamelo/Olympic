...
class OlympicScrapperNew:
    ...
    def extract_prizes_list(self, soup):
        ...
        for sc in scripts:
            if "organisation" in sc.text:  # Search key values from the Json
                json_text_match = re.search(r'\{.*\}', sc.text)  # Try to retrieve the fill Json
                if json_text_match:
        ...
        if "props" in data_json and "pageProps" in data_json["props"] and "initialMedals" in data_json["props"]["pageProps"] and "medalStandings" in data_json["props"]["pageProps"]["initialMedals"]:
            # Access into medalTable level
            medaltable = data_json["props"]["pageProps"]["initialMedals"]["medalStandings"].get('medalsTable', [])
           ...
           # Iterate through each row into medalTable
            for prz in medaltable:
                ...
                if isinstance(prz, dict) and 'organisation' in prz and prz['organisation']:
                    country = prz.get('organisation')  # Country ISO code
                    medals = prz.get('medalsNumber', [])
            
                    # Search the subdictionary with Total
                    total_medals = next((item for item in medals if item['type'] == 'Total'), None)
    ...
    def extract_event(self, soup):
        # Retrive Event Name
        self.event_name = next((img.get("alt") for img in soup.find_all("img", alt=True) if "Olympic" in img.get("alt")), None)
    ...
    def extract_medalists(self, soup):
        ...
        # Once known the levels, retrive the required data labels
        if "props" in medal_json and "pageProps" in medal_json["props"] and "initialMedallist" in medal_json["props"]["pageProps"]:
            medalist_table = medal_json["props"]["pageProps"]["initialMedallist"].get('athletes', [])
    ...
    def extract_medalist_event(self, soup):
        ...
        self.medalist_event_name = next((img.get("alt") for img in soup.find_all("img", alt=True) if "Olympic" in img.get("alt")), None)
        self.medalist_event_name = self.medalist_event_name.replace("Olympic Games ", "").strip() # Extract just city and year of the event
