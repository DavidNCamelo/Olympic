...
class medals_list:
    ...
    def __init__(self):
        ...
        self.country_df = pd.DataFrame(columns=["Country Code", "Country Name"]) # Initialize DataFrame for country codes and names
    ...
    def extract_country_list(self, soup):
        ...
        for mds in country:
            ...
            if country_name:
                ...
                self.country_df = pd.concat([self.country_df, pd.DataFrame({"Country Code": [ISO_code], "Country Name": [country_name_text]})], ignore_index=True)
    ...
    def extract_medals(self, soup):
        ...
        for row in medals_div:
            ...
            if len(current_country_medals) == 3:
                if self.country_code and self.cts_name:  # Ensure country lists are populated
                    country_info = {
                        'Event Name': self.name,
                        'Country Code': self.country_code.pop(0),  # Get the corresponding country code
